...
for idx, (band_name, band_info) in enumerate(bandsCW.items()):
...
ax.text(bandsCW['L1']['center'], bottom_offset + psd_values['L1']/2, 'L1',
        ha='center', va='center', fontweight='bold', color='white')

ax.text(bandsCW['L2']['center'], bottom_offset + psd_values['L2']/2, 'L2',
        ha='center', va='center', fontweight='bold', color='white')

ax.set_xlabel('Frequency (MHz)')
ax.set_ylabel('PSD (dBW/Hz)')
ax.set_title('CW Jamming in GPS Band')
...
save_figure(fig, 'jamming_gps_cw', formats=['png'])

### for barrage plot instead

bandsBarrage = {
    'Barrage': {'start': 1160.0, 'end': 1250.0, 'center': 1200.0},
    'L1': {'start': 1166.22, 'end': 1186.68, 'center': 1176.45},
    'L2': {'start': 1217.37, 'end': 1237.83, 'center': 1227.60},
}
...
figBarrage, axBarrage = plt.subplots()

colors = [get_color('first'), get_color('second'), get_color('third'), get_color('fourth')]
for idx, (band_name, band_info) in enumerate(bandsBarrage.items()):
    psd = psd_values[band_name]
    width = band_info['end'] - band_info['start']
    center = band_info['center']
    height = psd_values[band_name]
    
    axBarrage.bar(center, height, width=width, 
           color=colors[idx], alpha=0.7, 
           edgecolor='black', linewidth=1.5,
           label=f'{band_name} ({band_info["start"]:.2f}-{band_info["end"]:.2f} MHz), {(-180 + psd)} dBW', bottom = -180)
# For text positioning, add the bottom offset to your calculations
...
axBarrage.text(bandsBarrage['L1']['center'], bottom_offset + psd_values['L1']/2, 'L1',
        ha='center', va='center', fontweight='bold', color='white')

axBarrage.text(bandsBarrage['L2']['center'], bottom_offset + psd_values['L2']/2, 'L2',
        ha='center', va='center', fontweight='bold', color='white')

axBarrage.set_xlabel('Frequency (MHz)')
axBarrage.set_ylabel('PSD (dBW/Hz)')
axBarrage.set_title('Barrage Jamming in GPS Band')
axBarrage.set_xticks(np.arange(1160, 1250, 20))  # Ticks every 50 MHz
axBarrage.set_ylim(-180, -130)  # Adjust based on your data

# Add grid
axBarrage.grid(True, alpha=0.3, linestyle='--')
